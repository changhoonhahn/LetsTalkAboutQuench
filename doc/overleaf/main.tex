...
\author{Greg L. Bryan}
\affil{Flatiron Institute, 162 Fifth Avenue, New York NY 10010, USA}
\affil{Department of Astronomy, Columbia University, 550 W 120th Street, New York, NY 10027, USA}
\author{Ena Choi}
\affil{Department of Astronomy, Columbia University, 550 W 120th Street, New York, NY 10027, USA}
\author{Romeel Dav{\'e}}
...
varies by up to $\gtrsim 0.7\,\mathrm{dex}$ (factor of ${\gtrsim}5$) among the simulations with power-law 
slopes range from 0.7 to 1.2. In addition to the SFS, our GMM method also 
identifies sub-components in the star formation rate--stellar mass relation corresponding to star-burst, transitioning, and quiescent sub-populations.  
The hydrodynamic simulations show similar trends of these sub-populations 
versus mass, but qualitatively differ from that predicted in the SC-SAM. None of 
the hydrodynamic simulations reproduce the SDSS trend of GMM-identified quiescent 
galaxies with mass, while the SC-SAM trend agrees qualitatively. % but has too  few massive quiescent galaxies. 
All of the simulations also produce 
an abundance of low-mass quiescent galaxies not seen in observations.
These results illustrate that, even among models that well reproduce many other observables 
of the galaxy population, the $z=0$ SFS and other sub-populations still show 
marked differences that can provide strong constraints on galaxy formation models.
\end{abstract}
...
their distinct sub-grid model prescriptions. %Both as an effort to shed  light on the underlying physics and to validate their simulations and make predictions for new observations, 
...
galaxy sample to one or a few observational datasets. %(which can differ significantly as well as demonstrated in~\citealt{speagle2014}). 
Extending such 
...
{\color{red} %to compare galaxy populations across 
...
} % previously found agreement  among the SFSs of simulations and observations~\citep[\emph{e.g.}][]{somerville2015b}  is primarily driven by differences in how the SFS is identified  (Appendix~\ref{app:literature}).} 
...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{figs/Obvs_SFR_Mstar.pdf} 
...
every $10\,\mathrm{Myr}$. %The spatial,  mass, and temporal resolution of simulations can significantly impact the SFRs --- particularly the $100\,\mathrm{Myr}$ averaged SFRs in the hydrodynamic simulations. In Appendix~\ref{app:zerosfr}, we discuss how we treat these resolution effects in our analysis. 
...
%The full halo mass range of $2 \times 10^8$ to $3\times 10^{14}\ M_{\sun}$  is set by resolution (32 particles) at the low-mass end and by volume at  the high-mass end (e.g.~including 10 halos with $M>10^{14}M_{\sun}$). 
%It has a halo mass range of $2 \times 10^8$ to $3\times 10^{14}\ M_{\sun}$. 
...
Finite Mass (MFM) hydrodynamics method~\citep{hopkins2015a} rather than SPH.}  %{\sc Mufasa} is constructed  using {\sc Gizmo}, a code built  upon {\sc Gadget} where we employ the Meshless Finite Mass (MFM) hydrodynamics  method~\citep{hopkins2015a} rather than SPH.
...
gas within halos above a mass of $M_q>(1+0.48z)10^{12}M_\odot$%~\citep{mitra2015} 
...
We find no significant stellar mass dependence in the purities. %The SC-SAM, due to  its mass resolution, has purity of $44\%$ at $M_*$ below $10^{8.5}M_\odot$  and $97\%$ above. However, as we discuss in Section~\ref{sec:scsam}, we  impose a conservative stellar mass limit of $M_* > 10^{8.8} M_\odot$. 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.95\textwidth]{figs/Sims_SFR_Mstar.pdf} 
...
%We also find significant differences ($> 0.5\,\mathrm{dex}$) in the amplitudes  of the SFSs at $M_* = 10^{10.5} M_\odot$ ($b$ in Eq.~\ref{eq:powerlaw}). 
...
%observed SFR measurements, which would reduce the discrepancy. 
...
\caption{Components of the best-fit GMM for the SFR-$M_*$ relations of central galaxies 
in the Illustris, EAGLE, {\sc Mufasa}, and 
...
\caption{Fractional contributions, $\pi_i$, of the best-fit GMM components from 
    ...
    centrals from SDSS-VAGC and SDSS-NSA in the rightmost panel. In all the simulations, 
...
{\color{red}
For instance, if we treat the components below the SFS as quiescent %besides the high SF and SFS as quiescent 
...
seen in observations. %at ,in conflict with the $M_*$ lower bound of isolated quiescent galaxies in  \cite{geha2012}.
\end{itemize}
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.5\textwidth]{figs/Catalog_SFS_lit.pdf} 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.95\textwidth]{figs/Pssfr_GMMcomps_inst.pdf} 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.7\textwidth]{figs/Pssfr_res_impact.pdf} 
